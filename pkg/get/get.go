...
var release = map[string]ReleaseLocation{
...
func (tool Tool) GetURL(os, arch, version string, quiet bool) (string, error) {
	...
	if len(version) == 0 {
		...
		if _, supported := release[releaseType]; supported {
...
func FindRelease(location, owner, repo string) (string, error) {
	url := formatUrl(release[location].Url, owner, repo)

	clientTimeout := release[location].Timeout
	client := makeHTTPClient(&clientTimeout, false)
	client.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		return http.ErrUseLastResponse
	}

	req, err := http.NewRequest(release[location].Method, url, nil)
	if err != nil {
		return "", err
	}
	...
	if release[location].Method == http.MethodHead {
